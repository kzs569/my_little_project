...
preprocess(TRAIN_SETS_PATH, TRAIN_VOCAB_OUTPUT, TRAIN_DATA_OUTPUT)
preprocess(TEST_SETS_PATH, TEST_VOCAB_OUTPUT, TEST_DATA_OUTPUT)
preprocess(VALID_SETS_PATH, VALID_VOCAB_OUTPUT, VALID_DATA_OUTPUT)
...
TRAIN_DATA = "./tmp/ptb.train"          # 训练数据路径。
EVAL_DATA = "./tmp/ptb.valid"           # 验证数据路径。
TEST_DATA = "./tmp/ptb.test"            # 测试数据路径。
...
def run_epoch(session, model, batches, train_op, output_log, step, saver):
    # 计算平均perplexity的辅助变量。
    total_costs = 0.0
    iters = 0
    ...
    with tf.Session() as session:
        ...
        for i in range(NUM_EPOCH):
            ...
            step, train_pplx = run_epoch(session, train_model, train_batches,
                                         train_model.train_op, True, step, saver)
